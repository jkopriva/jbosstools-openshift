...
public class CreateResourcesTest extends AbstractTest  {
	...
	public void testCreateResourceFromLocalFile() {
		...
		assertTrue("Hello route has not been created from file", explorer.getOpenShift3Connection(connectionReq.getConnection())
...
//		assertTrue("Resource is not listed in created resources summary", new DefaultTree().getAllItems().size() == 1);
	...
	public void recreateProject() {
		if (explorer.getOpenShift3Connection(connectionReq.getConnection()).projectExists(testProject)) {
			explorer.getOpenShift3Connection(connectionReq.getConnection()).getProject(testProject).delete();
		}
	}
}
